<template>
  <div class="fixed top-3 right-0 m-6">
    <div
      :class="{
        'bg-red-200 text-red-900': data.type === 'error',
        'bg-green-200 text-green-900': data.type === 'success',
      }"
      class="rounded-lg shadow-md p-6 pr-10"
      style="min-width: 240px"
    >
      <button
      @click="close"
        class="opacity-75 cursor-pointer absolute top-0 right-0 py-2 px-3 hover:opacity-100"
      >
        ×
      </button>
      <div class="flex items-center">
        {{ $t(data.msg) }}
      </div>
    </div>
  </div>
</template>

<script>
export default {

  data() {
    return {
      message: {
        text: 'Hey! Something awesome happened.',
        type: 'success',
      },
    };
  },
  methods:{
      close(){
         this.$store.dispatch('ui/closeFlashMsg')

      }
  },
  props:['data']
};
</script>